...
const CLOUDFLARE_TARGET_DOMAIN =
  process.env.CLOUDFLARE_TARGET_DOMAIN || 'vault-tec-foundation.github.io';
...
async function createCNAMERecord(domain, target) {
  ...
  try {
    ...
    if (!data.success) {
      console.error(
        `Failed to create CNAME record: ${data.errors ? data.errors.map(e => e.message).join(', ') : 'Unknown error'}`
      );
...
async function deleteARecord(recordId) {
  ...
  try {
    ...
    if (!data.success) {
      console.error(
        `Failed to delete DNS record: ${data.errors ? data.errors.map(e => e.message).join(', ') : 'Unknown error'}`
      );
...
async function deleteCNAMERecordByName(domain) {
  try {
    const response = await fetch(
      `${CLOUDFLARE_API_URL}/zones/${CLOUDFLARE_ZONE_ID}/dns_records?type=CNAME&name=${domain}`,
      {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${CLOUDFLARE_API_TOKEN}`,
    ...
    if (!data.success) {
      console.error(
        `Failed to fetch CNAME record: ${data.errors ? data.errors.map(e => e.message).join(', ') : 'Unknown error'}`
      );
...
async function updateCNAMERecord(zoneId, recordId, newCNAME) {
  try {
    console.log(
      `Updating CNAME record ${recordId} with name: ${newCNAME.name}, content: ${CLOUDFLARE_TARGET_DOMAIN}`
    );
    ...
    if (!data.success) {
      console.error(
        `Failed to update CNAME record: ${data.errors ? data.errors.map(e => e.message).join(', ') : 'Unknown error'}`
      );
...
async function updateOrCreateCNAMERecord(domain, target) {
  try {
    console.log(
      `Updating or creating CNAME record for ${domain} pointing to ${CLOUDFLARE_TARGET_DOMAIN}`
    );
    const response = await fetch(
      `${CLOUDFLARE_API_URL}/zones/${CLOUDFLARE_ZONE_ID}/dns_records?type=CNAME&name=${domain}`,
      {
    ...
    if (!data.success) {
      console.error(
        `Failed to search for existing CNAME: ${data.errors ? data.errors.map(e => e.message).join(', ') : 'Unknown error'}`
      );
