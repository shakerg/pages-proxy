...
async function handleRepositoryEvent(payload) {
  ...
  if (action === 'created' || action === 'updated') {
    ...
    if (existingRecord && existingRecord.custom_domain) {
      console.log(
        `Deleting Cloudflare CNAME record for ${existingRecord.custom_domain} due to repository deletion`
      );
...
async function handlePagesEvent(payload) {
  ...
  try {
    ...
    if (action === 'created' || action === 'updated') {
      ...
      if (existingRecord && existingRecord.custom_domain) {
        console.log(
          `Pages site deleted/undeployed for ${repoName}. Removing custom domain: ${existingRecord.custom_domain}`
        );
        ...
        console.log(
          `Successfully removed CNAME record for ${existingRecord.custom_domain} from Cloudflare`
        );
      }
    }
  } catch (error) {
...
async function processCustomDomainChange(payload, existingRecord) {
  ...
  if (customDomain) {
    ...
    if (
      existingRecord &&
      existingRecord.custom_domain &&
      existingRecord.custom_domain !== customDomain
    ) {
      console.log(
        `Custom domain changed for ${repoName}. Old: ${existingRecord.custom_domain}, New: ${customDomain}`
      );
    ...
    const pagesUrl =
      pages?.html_url || (existingRecord ? existingRecord.pages_url : repository.html_url);
    ...
    console.log(
      `Custom domain removal detected for ${repoName}. Previous domain: ${existingRecord.custom_domain}`
    );
    ...
    console.log(
      `Successfully removed CNAME record for ${existingRecord.custom_domain} from Cloudflare and updated database`
    );
  }
}
...
async function handlePageBuildEvent(payload) {
  ...
  try {
    ...
    if (pagesResult && pagesResult.cname) {
      ...
      await db.storePagesUrl(
        repoName,
        repository.html_url || (existingRecord ? existingRecord.pages_url : null),
        customDomain
      );
      await cloudflare.updateOrCreateCNAMERecord(customDomain, 'foundation.redcloud.city');
    ...
    if (cnameContent) {
      ...
      if (existingRecord && existingRecord.custom_domain) {
        console.log(
          `Custom domain removal confirmed for ${repoName}. Previous domain: ${existingRecord.custom_domain}`
        );
        console.log(`Deleting Cloudflare CNAME record for ${existingRecord.custom_domain}`);
...
async function fetchCnameFile(repoName) {
  ...
  try {
    ...
    console.log(
      `Repository exists: ${repoData.full_name}, default branch: ${repoData.default_branch}`
    );
    ...
    if (!branches.includes(repoData.default_branch)) {
      try {
        ...
        if (error.status === 404) {
          ...
          console.error(
            'Error fetching CNAME file from default branch:',
